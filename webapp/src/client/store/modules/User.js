import {removeToken} from '@/client/config/tokenManager.js'
import { mobileDes, emailDes } from '../../../common/filter'
/*
 * mutations
 */
 ...
 */
const USER_STORAGE_KEY = `user@client`
...
const getters = {
  ...
  desensitizedName: state => {
    console.log('desensitizedName', state)
    if (state.mobile) {
      return mobileDes(state.mobile)
    } else if (state.email) {
      return emailDes(state.email)
    } else {
      return ''
    }
  }
